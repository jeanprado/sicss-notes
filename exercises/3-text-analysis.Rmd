...
# raw <- rtweet::search_tweets(q = "amazônia lang:pt", n = 10000,
...
```{r unnest_tokens}
tt <- raw |> dplyr::select(created_at, text) |> 
...
epa, pera. percebeu que essa expressão removeu também o ponto de números, aí `4.0` virou `40`? penso que pode ser um problema na análise, porque não representa exatamente. então vou remover todas as pontuações *menos* as que estão entre números.
...
inverse document frequency: útil para palavras pouco frequentes, mas aqui não mostrou muita coisa
```{r create_idf}
...
ainda quero testar com tweets (é possível? quais seriam outras covariáveis? autores? a interação entre os autores? como operacionalizar isso?), então por enquanto deixo algumas 
