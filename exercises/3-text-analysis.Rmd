...
agora que a diversão começa. usando o pacote `topicmodels` e seguindo as instruções [dessa aula aqui](https://sicss.io/2020/materials/day3-text-analysis/topic-modeling/rmarkdown/Topic_Modeling.html), usando Latent Dirichlet Allocation 

(depois que rodei o modelo, além de mudar o valor de `k` repetidas vezes, vi que talvez seja frutífero remover a palavra `amazônia` antes da criação do `dtm`, já que ela é comum a todos os grupos -- é dela que puxamos os tweets)

(a regex `amaz[ôóo]nia` filtra essas 3 versões: amazônia, amazónia e amazonia)
