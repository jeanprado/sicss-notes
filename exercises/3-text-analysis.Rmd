...
para remover as URLs do texto, usei uma [regex](https://pt.wikipedia.org/wiki/Express%C3%A3o_regular) que copiei [dessa thread](https://stackoverflow.com/questions/3809401/what-is-a-good-regular-expression-to-match-a-url) no stack overflow. (ninguém é de ferro, né?)
...
suspeito que essas palavras mais raras são na verdade as @s de usuários com poucas interações/tweets sobre amazônia. então, antes de rodar o algoritmo da tf-idf, vou remover as linhas que contém nomes de usuários presentes no dataset.
```{r remake_idf}
# se existe algum jeito menos burro de combinar múltiplas colunas em um vetor de caracteres, desconheço
...
agora que a diversão começa. usando o pacote `topicmodels` e seguindo as instruções [dessa aula aqui](https://sicss.io/2020/materials/day3-text-analysis/topic-modeling/rmarkdown/Topic_Modeling.html), usando Latent Dirichlet Allocation (LDA)
